...
/* minimum height of menu lines */
static unsigned int lineheight = 0;           /* default 0, overridden by -h */
...
unsigned long parse_hex_color(const char *hex) {
  ...
  if (hex[0] == '#')
    hex++; // skip #
  if (sscanf(hex, "%lx", &color) != 1) {
    fprintf(stderr, "Invalid color: %s\n", hex);
    return 0xff0000; // fallback to red
  }
  return color;
}
...
int main(int argc, char *argv[]) {
  ...
  for (i = 1; i < argc; i++)
    ...
    /* these options take one argument */
      ...
      border_color = parse_hex_color(argv[++i]);
    else
      usage();
